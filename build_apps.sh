...
if [ $BUILD == 2 ]; then
  docker build --platform linux/amd64 . -f ./$APP_GROUPNAME/Dockerfile --no-cache -t $APP_GROUPNAME:latest
elif [ $BUILD == 1 ]; then # find the existing cache/image and start from there
  docker build --platform linux/amd64 . -f ./$APP_GROUPNAME/Dockerfile -t $APP_GROUPNAME:latest
fi
...
case $APP_GROUPNAME in
  solr)
    ...
    if [ $( docker ps -a -f name=web_search_dataset | wc -l ) -eq 2 ]; then
      ...
      docker run --platform linux/amd64 --name web_search_dataset cloudsuite/web-search:dataset
    fi
    ...
    docker exec -it -d --privileged $APP_GROUPNAME\_$USER /bin/bash -c '(docker run --platform linux/amd64 -it --name web_search_client --net host cloudsuite/web-search:client $(hostname -I) 10; pkill java)'
    docker run --platform linux/amd64 -e user_id=$USER_ID -e group_id=$GROUP_ID -e username=$USER -e HOME=/home/$USER -dit --privileged --name $APP_GROUPNAME\_$USER --mount type=bind,source=$OUTDIR,target=/home/$USER $APP_GROUPNAME:latest /bin/bash
  ...
  sysbench)
    ...
    ;;
  allbench_traces)
    docker run --platform linux/amd64 -e user_id=$USER_ID -e group_id=$GROUP_ID -e username=$USER -e HOME=/home/$USER -dit --privileged --name $APP_GROUPNAME\_$USER --mount type=bind,source=/soe/hlitz/lab/traces,target=/simpoint_traces,readonly --mount type=bind,source=$OUTDIR,target=/home/$USER $APP_GROUPNAME:latest /bin/bash
    docker start $APP_GROUPNAME\_$USER
    docker exec --privileged $APP_GROUPNAME\_$USER /bin/bash -c "/usr/local/bin/common_entrypoint.sh"
    docker exec --user=$USER --privileged $APP_GROUPNAME\_$USER /bin/bash -c "cd /home/$USER/scarab/src && make clean && make"
    ;;
